...
class AmericanBinomialTreeModel:
    ...
    def calculate_american_option_prices(self, price_matrix):
    ...
    def OptionsVal(self, n, S, K, r, v, T, priceMatrix):
        ...
        for i in range(n+1):
            putMatrix[i, n] = max(K - priceMatrix[i, n], 0)
            callMatrix[i, n] = max(priceMatrix[i, n] - K, 0)
        for j in range(n, 0, -1):
            for i in range(j):
                putMatrix[i, j-1] = max(K - priceMatrix[i, j-1], (p *
                                        putMatrix[i, j] + (1-p) * putMatrix[i+1, j]) * m.exp(-r * dt))
                callMatrix[i, j-1] = max(priceMatrix[i, j-1] - K, (p * callMatrix[i, j] + (
                    1-p) * callMatrix[i+1, j]) * m.exp(-r * dt))
